...
def draw_dialogue(page, col_right, col_left, text):
    """セリフを縦書き配置（枠に収まらない場合はフォントサイズを縮小して収める）"""
    if not text:
        return
    ...
    # 利用可能な行数（幅）を計算
    avail_lines = max(1, int((col_right - col_left) / lw))
    
    # 収まるまでフォントサイズを縮小
    # 文字を小さくすることで、1行あたりの収容文字数を増やし、行数を減らす
    while fs > 8.0:
        chars_per_col = int((DLG_END - DLG_Y) / sp)
        vlines = smart_split(text, chars_per_col)
        if len(vlines) <= avail_lines:
            break
        fs -= 0.5
        sp = fs # 縮小時は文字間隔もフォントサイズに合わせる

    # 決定したフォントサイズで描画
    chars_per_col = int((DLG_END - DLG_Y) / sp)
    vlines = smart_split(text, chars_per_col)
    
    x_center = col_right - lw / 2
    for vl in vlines:
        if x_center - lw / 2 < col_left - 1: # わずかな誤差を許容
            break
        vtext(page, x_center, DLG_Y, vl, fontsize=fs, char_sp=sp)
        x_center -= lw


def draw_name(page, col_right, col_left, name):
    """人物名を縦書き配置（枠に収まらない場合はフォントサイズを縮小して1行に収める）"""
    if not name:
        return
    
    # 人物名エリアの高さ
    max_h = Y_CHAR_BTM - 4 - CHAR_Y
    
    # 1行に収まるフォントサイズを計算
    fs = FONT_SIZE
    sp = CHAR_SP
    # 文字数 * 間隔 が高さを超える間、フォントサイズと間隔を小さくする
    while len(name) * sp > max_h and fs > 7.0:
        fs -= 0.5
        sp = fs # 縮小時は文字間隔もフォントサイズに合わせる
        
    x_center = col_right - LINE_W / 2
    vtext(page, x_center, CHAR_Y, name, fontsize=fs, char_sp=sp)
